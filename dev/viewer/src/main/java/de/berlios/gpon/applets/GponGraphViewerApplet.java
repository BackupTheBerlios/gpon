 ...
 * 
 */
public class GponGraphViewerApplet extends JApplet {
	...
	final Color colorMap[] = new Color[] { new Color(240, 240, 255),
			Color.GREEN, Color.BLUE, Color.CYAN, Color.MAGENTA, Color.ORANGE,
	...
	public void start() {
		...
		VertexStrokeHighlight vsh = new VertexStrokeHighlight(vv
				.getPickedState());
		...
		final MyDetailButton detailButton = new MyDetailButton(
				"Detail (select node)");
		...
		detailButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					String objectId = (String) graphExport.getPickedVertex()
							.getUserDatum("objectId");
					japplet.getAppletContext().showDocument(
							new URL(detailURL + objectId), detailTarget);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
		...
		final JButton newButton = new JButton("Neighbours");
		...
		newButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					try {
						// i.e:
						// "http://localhost:9080/wui/exploration/environment.do?radius=1&objectId="
						String urlString = gvConfig.getGraphExplorationUrl()
								+ objectId;
		...
		vv.getPickedState().addItemListener(new ItemListener() {
			private void vertexPicked(
					edu.uci.ics.jung.graph.ArchetypeVertex vertex) {
			...
			public void itemStateChanged(ItemEvent e) {
				if (e.getItem() instanceof ArchetypeVertex) {
					ArchetypeVertex vertex = (ArchetypeVertex) e.getItem();
			...
			public boolean evaluate(Object input) {
				try {
					if (input instanceof ArchetypeEdge) {
						return ((ArchetypeEdge) input).getUserDatum("type")
								.equals("inheritance");
					}
				} catch (Throwable t) {
				}
				...
				if (gvConfig.isNodeTypeColors()) {
					...
					if (v.getUserDatum("nodeType") != null) {
						...
						if (vertexColorsByType.containsKey(nodeType)) {
							...
							// colors before
							int noOfColors = vertexColorsByType.size();
		...
		pr.setEdgeLabelClosenessFunction(new ConstantDirectionalEdgeValue(0.5,
				0.5));
		...
		if (gvConfig.isNodeDetails()) {
			pr.setVertexShapeFunction(new VertexShapeFunction() {
				public Shape getShape(Vertex v) {
					VertexShapeFactory vsf = new VertexShapeFactory(myVsf,
							myVarf);
			...
			pr.setVertexStringer(new VertexStringer() {
				public String getLabel(ArchetypeVertex v) {
					StringBuffer label = new StringBuffer("<html><body>"
							+ v.getUserDatum("name") + "<hr>");
		...
		final VisualizationViewer.Paintable preRenderer = new VisualizationViewer.Paintable() {
			public void paint(Graphics g) {
				printLogo(g);
			}
		...
		// Font
		pr.setVertexFontFunction(new ConstantVertexFontFunction(Font
				.getFont("Arial")));
		...
		plus.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				graphMouse.mouseWheelMoved(new MouseWheelEvent(vv,
						MouseEvent.MOUSE_WHEEL, System.currentTimeMillis(), 0,
						d.width / 2, d.height / 2, 1, false,
		...
		final JComboBox xCombo = new JComboBox(new String[] { "800", "1024",
				"1600", "2400" });
		final JComboBox yCombo = new JComboBox(new String[] { "600", "768",
				"1200", "1800" });
		...
		export.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int desiredX = new Integer(xCombo.getSelectedItem().toString())
						.intValue();
				int desiredY = new Integer(yCombo.getSelectedItem().toString())
						.intValue();

				BufferedImage image = (BufferedImage) vv.createImage(desiredX,
						desiredY);

				Graphics2D g = image.createGraphics(); // Get a Graphics2D
														// object
				...
				Dimension oldPreferredLayoutSize = vv.getGraphLayout()
						.getCurrentSize();
				...
				MutableAffineTransformer vmat = new MutableAffineTransformer(
						vat);

				MutableAffineTransformer lmat = new MutableAffineTransformer(
						lat);
				...
				// vv.getViewTransformer().scale(1.2,1.2,new
				// Point2D.Double(1000,500));
				...
				try {
					...
					PostMethod httppost = new PostMethod(new URL(postURL)
							.toExternalForm());

					httppost.addParameter("keybase", keybase);

					Part[] parts = { new FilePart("file",
							new ByteArrayPartSource("foo", out)) };
					...
					try {
						...
						if (httppost.getStatusCode() == HttpStatus.SC_OK) {
							...
							japplet.getAppletContext().showDocument(
									new URL(displayURL + "keybase=" + keybase),
									"_blank_");
	...
	public static void main(String[] args) {
		...
		GponGraphViewerApplet gzspd = new GponGraphViewerApplet();

		gzspd.gvConfig = GraphViewerConfiguration
				.getDebugConfiguration("node.details=off node.typecolor=on");
	...
	public void printLogo(Graphics g) {
		...
		String[] lines = new String[] { "GPON graph viewer applet" };
		g.setFont(new Font("Helvetica", Font.PLAIN, fontSize));
		g.setColor(new Color(96, 150, 191,100));
	...
	public void init() {
		try {
			...
			this.gvConfig = GraphViewerConfiguration
...
interface MyVertexAspectRatioFunction extends VertexAspectRatioFunction {
...
class MyDetailButton extends JButton {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -5846538786318479808L;
