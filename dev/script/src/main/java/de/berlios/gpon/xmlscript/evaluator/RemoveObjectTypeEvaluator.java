...
public class RemoveObjectTypeEvaluator extends AbstractEvaluator {
	
	public Object evaluate(XmlObject xmlObject, ScriptContext sc, CallData cd) {
		
		RemoveObjectType removeObjectType =
			(RemoveObjectType)xmlObject;
		
		ItemType it =
			sc.getModelDao().findItemTypeByName(removeObjectType.getName());
		
		if (it==null) 
		{
			log.warn("Item type "+removeObjectType.getName()+"does not exist.");
		}
		else 
		{
