...
public class DefaultPluggableProcessor 
	implements Processor
{
	...
	public GponScriptOutput process(GponScript script) { 
		...
		ScriptContext sc = 
			(ScriptContext)LookupService.getBeanForId("DefaultScriptContext"); 
		...
		GponScriptOutputDocument outDoc =
			GponScriptOutputDocument.Factory.newInstance();
		...
		while (it.hasNext()) 
		{
			...
			System.out.println("Id: "+name+" size: "+((set!=null)?set.size():0));
			
			de.berlios.gpon.xmlscript.Set outputSet =
				out.addNewSet();
			...
			while (setIterator.hasNext()) 
			{
				...
				de.berlios.gpon.xmlscript.Object outputObject =
					outputSet.addNewObject();
