...
import de.berlios.gpon.xmlscript.Intersection;
import de.berlios.gpon.xmlscript.ScriptContext;


public class IntersectionEvaluator implements Evaluator {

	public Object evaluate(XmlObject xmlObject, ScriptContext sc, CallData cd) {
		System.out.println("[Intersection] calling And");
		Set set = (Set)new AndEvaluator().coreEvaluate(xmlObject,sc,cd);
		
		Intersection intersection =
			(Intersection)xmlObject;
		
		if (intersection.getId()!=null) {
			sc.setAttribute(intersection.getId(),set);
		}
